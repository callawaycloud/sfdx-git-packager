...
export default class Package extends SfdxCommand {
    ...
    public async run(): Promise<AnyJson> {
        ...
        this.sourcePaths = (
            (await this.project.resolveProjectConfig())["packageDirectories"] as Array<{ path: string }>
        ).map((d) => d.path);
        ...
        try {
            ...
            try {
                const stat = await fs.lstatSync(outDir);
                if (stat.isDirectory()) {
                    let purge = false;
                    if (this.flags.purge) {
                        purge = true;
    ...
    private async setupTmpProject(changed: string[], sourceRef: string | undefined) {
        ...
        for (const sourcePath of this.sourcePaths) {
            await mkdirp(join(tempDir, sourcePath));
        }
        ...
        for (const path of changed) {
            ...
            for (const resolved of resolvedMetadata) {
                ...
                await mkdirp(dirname(newPath));
                if (typeof resolved.source === "string") {
                    fs.writeFileSync(newPath, resolved.source);
                } else {
