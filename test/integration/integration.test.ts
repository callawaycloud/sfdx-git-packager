...
async function runTest(testName: string, params?: string[]) {
    ...
    try {
        ...
        const compareRes = compareSync(
            "test/integration/project/deploy",
            `test/integration/output/${expectedOutputDir}`,
            { compareContent: true }
        );
        const mismatched = compareRes.diffSet?.filter((diff) => diff.state !== "equal") || [];

        for (const mismatch of mismatched) {
            const file1 = mismatch.path1 + "/" + mismatch.name1;
            const file2 = mismatch.path2 + "/" + mismatch.name2;
            console.log("Diffing files:", file1, file2);
            const diff = exec(
                `diff ${mismatched[0].path1}/${mismatched[0].name1} ${mismatched[0].path2}/${mismatched[0].name2}`
            );
            diff.stdout?.on("data", function (data) {
                console.log(data);
            });

            // uncomment to copy the test file to the output directory (use if you want to update the test)
            //exec(`cp ${file1} ${file2}`);
        }

        const mismatchedNames = mismatched.map((diff) => diff.name1 || diff.name2) || [];
        assert.strictEqual(
            mismatchedNames.length,
            0,
            `${mismatchedNames.length} file(s) were different: \n${mismatchedNames.join("\n")}`
        );
